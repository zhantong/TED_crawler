...
class TED(baseClass.BaseClass):
    ...
    def dl_from_api(self):  # 从官网API接口获得全部视频信息
        ...
        # 将所有视频英文信息存储到json格式文件
        self.write_utf8('ted_json', self.json_dump(self.talk_info))

    # 将获得的英文视频信息存储至数据库，每次都会删除原先的数据库，建立新的数据库，这样来确保数据库中内容均为最新
        ...
        with codecs.open(file_name, 'w', 'utf-8') as f:
            for key in sorted(file_list.keys()):
                ...
                for file in os.listdir(file_list[key]):
                    f.write('|    ├─' + file + '\r\n')
        print('目录树生成成功，已经存储到%s' % (path + '/' + file_name))
